...
//		updateSeatUtils.checkExceptions(employee, newEmployeeDto);
	...
	public SeatInformationViewDto seatinformationByID(int id) {
		...
		SeatNumber seatNumber = this.seatNumberRepo.findById(id).get();
	...
	public List<EmployeeDto> searchByName(String name, Pageable pageable) {
		return employeeRepo.searchByName(name,pageable).stream().map((employee)->this.modelMapper.map(employee, EmployeeDto.class)).collect(Collectors.toList());
	}
